...
-- one trillion (1012) routes every second it would take over twenty billion years to check
-- them all. There is an efficient algorithm to solve it. ;o)

import System.IO

myMax r = zipWith max (init r) (tail r)
p67 = foldr1 (\row acc -> zipWith (+) row (myMax acc))
...
main = do
    ...
    let solution = p67 $ toInt $ map words (lines contents)
    putStr (show $ head solution)
